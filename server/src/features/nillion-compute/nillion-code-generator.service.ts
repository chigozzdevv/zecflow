...
class NillionCodeGeneratorService {
  generateNodeJsCode(graph: NillionBlockGraph, workflowInputs: Record<string, any>): string {
    ...
    codeLines.push('console.log("[WORKFLOW] Files in /app:", fs.existsSync("/app") ? fs.readdirSync("/app") : "dir not found");');
    codeLines.push('');
    ...
    codeLines.push('');
    codeLines.push('const inputData = JSON.parse(fs.readFileSync("/app/input.json", "utf8"));');
    codeLines.push('');
    ...
    codeLines.push('  }');
    codeLines.push('');
    codeLines.push('  if (req.url === "/output.json" || req.url === "/") {');
    ...
    codeLines.push('  }');
    codeLines.push('');
    codeLines.push('  if (req.url === "/health") {');
    codeLines.push('    res.writeHead(200);');
    ...
    codeLines.push('      console.log("Execution completed successfully");');
    codeLines.push('    })');
    codeLines.push('    .catch(error => {');
    codeLines.push('      console.error("Execution failed:", error);');
